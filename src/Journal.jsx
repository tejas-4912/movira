...
const MOODS = [
  ...
  { label: 'Bad', emoji: '😣', color: 'text-red-400' },
]
...
function PainDot({ level }) {
  ...
  return <span className={`inline-block w-3 h-3 rounded-full ${colors[level] || 'bg-slate-600'}`}></span>
}
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="lg:ml-56 px-6 lg:px-10 py-8 max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className="text-teal-400 text-xs font-semibold uppercase tracking-widest mb-1">Daily Tracking</p>
            <h1 className="text-3xl font-bold">Pain Journal</h1>
            <p className="text-slate-400 text-sm mt-1">Track your pain levels and recovery notes daily</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">Avg Pain (7 days)</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">Pain Trend</p>
          </div>
        </div>

        {/* Log form */}
        {showForm && (
          <div className="bg-slate-900 border border-teal-700 rounded-2xl p-6 mb-8">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
                  className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-teal-500" />
              </div>
              <div>
                <label className="block text-sm text-slate-300 mb-2">Pain Level: <strong className="text-white">{form.painLevel}/10</strong></label>
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-2">Pain Location</label>
                <div className="flex flex-wrap gap-2">
                  {LOCATIONS.map(loc => (
                    <button key={loc} onClick={() => setForm(f => ({ ...f, painLocation: loc }))}
                      className={`px-3 py-1.5 rounded-lg text-sm border transition-colors ${form.painLocation === loc ? 'bg-teal-500/20 border-teal-500 text-teal-300' : 'bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-500'}`}>
                      {loc}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-2">How are you feeling?</label>
                <div className="flex gap-3">
                  {MOODS.map(m => (
                    <button key={m.label} onClick={() => setForm(f => ({ ...f, mood: m.label }))}
                      className={`flex flex-col items-center gap-1 p-2 rounded-xl border transition-colors ${form.mood === m.label ? 'border-teal-500 bg-teal-500/10' : 'border-slate-700 hover:border-slate-500'}`}>
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-2">Notes <span className="text-slate-500">(optional)</span></label>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => setShowForm(false)}
                  className="border border-slate-700 text-slate-400 px-6 py-2.5 rounded-xl text-sm transition-colors hover:border-slate-500">
                  Cancel
                </button>
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {entries.map((entry, i) => {
              ...
              return (
                <div key={i} className="bg-slate-900 border border-slate-800 hover:border-slate-600 rounded-2xl p-5 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <span className="text-2xl">{mood?.emoji || '😐'}</span>
                      <div>
                        <p className="font-semibold text-white">{new Date(entry.date).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
                        <p className="text-xs text-slate-400">{entry.painLocation} · Mood: {entry.mood}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2">
                        <PainDot level={entry.painLevel} />
                        <span className="text-sm font-bold text-white">{entry.painLevel}/10</span>
                      </div>
                      <button onClick={() => handleDelete(entry.date)} className="text-slate-600 hover:text-red-400 text-lg transition-colors">×</button>
                    </div>
                  </div>
                  {entry.notes && <p className="text-sm text-slate-400 leading-relaxed border-t border-slate-800 pt-3">{entry.notes}</p>}
                </div>
              )
            })}
          </div>
